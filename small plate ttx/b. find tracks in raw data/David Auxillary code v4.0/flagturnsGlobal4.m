function final_tracks=flagturnsGlobal4(tr)
%
% General: 
...
%                   tracks(i).run_velocities = velocities of runs (pixel/sec)
...
%                   tracks(i).run_err = cummulative error for each run (pixels)
%
%               Isothermal Tracks:
...
%                   tracks(i).IT_dist = end-to-end distance (NOT along curve) in pixels for every IT
%                   tracks(i).IT_Int_along_path = distance along curve, in pixels, for each IT 
%                   tracks(i).IT_time = time in seconds of individual ITs
%                   tracks(i).IT_num = total number of ITs
%                   tracks(i).IT_total_time = total time of ITs
%                   tracks(i).IT_total_frames = total number of frames of ITs
%                   tracks(i).IT_total_dist = total distance (pixels) of ITs 
%                   tracks(i).IT_velocities = velocities (pixels/sec) during ITs
%                   tracks(i).IT_X_location =  average X-axis location of each IT
%                   tracks(i).IT_temperatures = average Temperature of each IT
%%%%%%%%%%%%%%%%%%%%%%%%%% 
...
for i=1:length(tr) %for each track of each worm
    ...
    for k = 1:Ns % find groups that are larger than N_SEG runs, calculate angles.. 
        if segs(k,3)==cur_gr
            ...
            if cur_gr_size >= N_SEG % "run" includes segments (cur_first)..(k-1), all in cur_gr
                ...
                if (dx^2+dy^2)>0                    
                    vec_run = [dx, dy];
                    x0 = x(run_indx(Nr,1)); % X - starting point of current run
                    y0 = y(run_indx(Nr,1)); % Y - starting point of current run
                    for pt_in_run = (run_indx(Nr,1)+1):run_indx(Nr,2) % indices of points belonging to current run
                        x1 = x(pt_in_run); % X - current raw data point in run 
                        y1 = y(pt_in_run); % Y - current raw data point in run 
                        vec_pt = [x1-x0, y1-y0]; % vector from run start to point
                        pt_err = (vec_pt(1)*vec_run(1)+vec_pt(2)*vec_run(2))^2; % square dot product
                        pt_err = pt_err/(vec_run(1)^2+vec_run(2)^2); % square projection of point on run
                        pt_err = (vec_pt(1)^2+vec_pt(2)^2)-pt_err; % square distance of point from run
                        pt_err = max(pt_err,0); % ignore small rounding errors
                        %if pt_err<0 
                        %    disp(['Error: pt_err<0 at track ' num2str(i) ' , run ' num2str(Nr)]);
                        %    disp(['       pt_err = ' num2str(pt_err)]);
                        %end;
                        pt_err = sqrt(pt_err); 
                        cur_err = cur_err+pt_err; % add current point to current run error
                    end;
                end; % if (run_dx^2+run_dy^2)>0
                run_err = [run_err; cur_err]; % column vector of errs in pixels 
                
            end; % if cur_gr_size >= N_SEG
            cur_gr = segs(k,3);
            cur_first = k;
            cur_gr_size = 1; 
    ...
    tracks(i).run_duration = run_dur; % column vector of durations (# of frames) for each run
    ...
    if ~isempty(run_time)
        tracks(i).run_velocities = run_Int_along_path ./ run_time; % velocities of runs (pixel/sec)
    else 
        tracks(i).run_velocities = [];
    end;
    tracks(i).curved = run_Int_along_path > (pi/2 * run_dist); % column vector, Alex/Damon criteria for curved run
    tracks(i).run_num = Nr;
    tracks(i).run_err = run_err; % cummulative error for each run (pixels)
     px2mm = 85/abs(tracks(i).max_edge - tracks(i).min_edge); %85mm per plate / edges 
    %%% save Isothermal Tracks %%%
    IT_index = find( (abs(abs(run_ang)-pi/2) < MAXDANG_IT) & ((run_dist.*px2mm) > MINLENGTH_IT) );
    tracks(i).IT_indx = run_indx(IT_index,:); % start frame index and end frame index of each IT
    tracks(i).IT_duration = run_dur(IT_index); % duration (# of frames) of each IT
    tracks(i).IT_angle = run_ang(IT_index); % angle (radians) of each IT
    tracks(i).IT_dx = run_dx(IT_index); % x-axis projection (pixels) of each IT
    tracks(i).IT_dy = run_dy(IT_index); % y-axis projection (pixels) of each IT
    tracks(i).IT_dist = run_dist(IT_index); % end-to-end distance (pixels) traveled in each IT
    tracks(i).IT_Int_along_path = run_Int_along_path(IT_index); % distance along curve (pixels) traveled in each IT
    tracks(i).IT_time = run_time(IT_index); % time (seconds) of each IT
    tracks(i).IT_num = length(IT_index); % total number of ITs
    if tracks(i).IT_num > 0 % there are ITs in current track
        tracks(i).IT_total_time = sum(run_time(IT_index));
        tracks(i).IT_total_frames = sum(run_dur(IT_index));
        tracks(i).IT_total_dist = sum(run_dist(IT_index)); 
        tracks(i).IT_velocities = tracks(i).IT_Int_along_path ./ tracks(i).IT_time;
        tracks(i).IT_X_location = (x(tracks(i).IT_indx(:,1))+x(tracks(i).IT_indx(:,2)))/2; % average location of each IT
        
       % slp = (tracks(i).max_T - tracks(i).min_T)/abs(tracks(i).max_edge - tracks(i).min_edge);
       % intrcpt = tracks(i).max_T - (slp * tracks(i).max_edge);
        
        tracks(i).IT_temperatures = interp1(linspace( tracks(i).min_edge,tracks(i).max_edge,100) ...
                                           ,linspace(tracks(i).min_T,tracks(i).max_T ,100),tracks(i).IT_X_location,'linear');
        %tracks(i).IT_temperatures = slp.*(tracks(i).IT_X_location) + intrcpt; % average Temperature of each IT
        tracks(i).IT_X_postions = tracks(i).IT_X_location;
    else % No ITs in current track
        tracks(i).IT_total_time = 0;
        tracks(i).IT_total_frames = 0;
        tracks(i).IT_total_dist = 0; 
        tracks(i).IT_velocities = [];
        tracks(i).IT_X_location = []; % average location of each IT
        tracks(i).IT_temperatures = []; % average Temperature of each IT
        tracks(i).IT_X_postions = []; % average Temperature of each IT
    end;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
