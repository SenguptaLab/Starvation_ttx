function final_tracks=flagturnsLocal4(tr)
%
% General: 
...
for i=1:length(tr) %for each track of each worm
    ...
    %tracks(i).run_dx_mm = run_dx*85/abs(tracks(i).max_edge - tracks(i).min_edge);
    tracks(i).run_dy = run_dy; % column vector of total y-axis distance (in pixels) for each ru
    %tracks(i).run_dy_mm = run_dy*85/abs(tracks(i).max_edge - tracks(i).min_edge);
    tracks(i).run_dist = run_dist;  % column vector of end-to-end distance (in pixels) for each ru
    %tracks(i).run_dist_mm = run_dist*85/abs(tracks(i).max_edge - tracks(i).min_edge);
    ...
    %tracks(i).IT_dx_mm = run_dx_mm(IT_index);
    tracks(i).IT_dy = run_dy(IT_index);
    %tracks(i).IT_dy_mm = run_dy(IT_index);
    tracks(i).IT_dist = run_dist(IT_index);
    %tracks(i).IT_dist_mm = run_dist_mm(IT_index); 
