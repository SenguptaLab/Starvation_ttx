function out=putinfields4(in, clean, plate_id, cal_data, tot_time, fn_id, data_path, varargin)
...
%         out(i).plate_id = a number plate identifier for the plate of origin of particle i
%         out(i).file_name_id = the file-name plate identifier for the plate of origin of particle i
%         out(i).path = path for data from the plate of origin of particle i
...
for i=1:length(u) % transfer in array to out structure for each particle (i)
    ...
    out(i).plate_id = plate_id; % a number plate identifier for the plate of origin of particle i
    out(i).file_name_id = fn_id; % the file-name plate identifier for the plate of origin of particle i
    out(i).path = data_path; % path for data from the plate of origin of particle i
   
    if(length(cal_data)>4) %image of plate is reversed #MB added for reversed plates 
    out(i).min_edge = cal_data(2); % pixel value of edge with minimal Temperature
    out(i).max_edge = cal_data(1); % pixel value of edge with maximal Temperature
    out(i).reversed = 1;
    else
    ...
    for i=1:length(out)
        ...
        if ~isempty(split)
            for j=1:length(split)-1
                ...
                out(new).plate_id = plate_id; % a number plate identifier for the plate of origin of particle i
                out(new).file_name_id = fn_id; % the file-name plate identifier for the plate of origin of particle i
                out(new).path = data_path; % path for data from the plate of origin of particle i
                    if(length(cal_data)>4) %image of plate is reversed #MB added for reversed plates
                out(new).min_edge = cal_data(2); % pixel value of edge with minimal Temperature
                out(new).max_edge = cal_data(1); % pixel value of edge with maximal Temperature
                out(new).reversed = 1;
                    else
